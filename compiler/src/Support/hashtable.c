...
void hashtable_extend(hashtable_t *table)
{
    ...
    ht_foreach(table, entry)
    {
        ...
        hashtable_insert(&new, entry->key_hash, entry->value);
    }
...
void hashtable_insert(hashtable_t *table, uint64_t key_hash, void *value)
{
    ...
    size_t index = key_hash % table->capacity;
    ...
    while (1)
    {
        ...
        else if (entry->state == HT_ENTRY_USED && entry->key_hash == key_hash)
        {
            entry->value = value;
            return;
        }
...
static hashtable_entry_t *hashtable_get_entry(hashtable_t *table, uint64_t key_hash)
{
    ASSERT(table);
    ...
    {
        if (entry->state == HT_ENTRY_USED && entry->key_hash == key_hash)
            return entry;
...
{
    hashtable_entry_t *entry = hashtable_get_entry(table, key_hash);
    if (!entry)
        return NULL;
